# Phase 3: DM 캐싱, 프로필 캐싱, 성능 최적화 계획

## 목표

기존 게시글/댓글 캐싱에 이어 DM과 프로필 캐싱을 추가하고, 전체 캐시 시스템의 성능을 최적화합니다.

---

## 1. DM 캐싱 구현

### 1.1 현재 상태 분석

**기존 구현** ([lib/services/dm_service.dart](lib/services/dm_service.dart)):
```dart
// 메모리 캐시만 존재 (TTL 없음)
final Map<String, Conversation> _conversationCache = {};
final Map<String, List<DMMessage>> _messageCache = {};
```

**문제점:**
- 메모리 전용 (앱 종료 시 손실)
- TTL 없음 (무제한 메모리 사용)
- 디스크 캐시 없음 (오프라인 불가)
- 무효화 로직 없음

### 1.2 구현 계획

#### 1.2.1 모델에 toMap/fromMap 추가

**파일:** [lib/models/conversation.dart](lib/models/conversation.dart)

```dart
// 캐싱용 Map 변환 메서드 추가
Map<String, dynamic> toMap() {
  return {
    'id': id,
    'participants': participants,
    'participantNames': participantNames,
    'participantPhotos': participantPhotos,
    'isAnonymous': isAnonymous,
    'lastMessage': lastMessage,
    'lastMessageTime': lastMessageTime.millisecondsSinceEpoch,
    'lastMessageSenderId': lastMessageSenderId,
    'unreadCount': unreadCount,
    'postId': postId,
    'dmTitle': dmTitle,
    'createdAt': createdAt.millisecondsSinceEpoch,
    'updatedAt': updatedAt.millisecondsSinceEpoch,
    'archivedBy': archivedBy,
    'userLeftAt': userLeftAt.map((k, v) => MapEntry(k, v?.millisecondsSinceEpoch)),
    'rejoinedAt': rejoinedAt.map((k, v) => MapEntry(k, v?.millisecondsSinceEpoch)),
  };
}

factory Conversation.fromMap(Map<String, dynamic> map, String id) {
  // fromFirestore와 유사하지만 millisecondsSinceEpoch 사용
}
```

**파일:** [lib/models/dm_message.dart](lib/models/dm_message.dart)

```dart
Map<String, dynamic> toMap() {
  return {
    'id': id,
    'senderId': senderId,
    'text': text,
    'createdAt': createdAt.millisecondsSinceEpoch,
    'isRead': isRead,
    'readAt': readAt?.millisecondsSinceEpoch,
  };
}

factory DMMessage.fromMap(Map<String, dynamic> map, String id) {
  return DMMessage(
    id: id,
    senderId: map['senderId'] ?? '',
    text: map['text'] ?? '',
    createdAt: DateTime.fromMillisecondsSinceEpoch(map['createdAt']),
    isRead: map['isRead'] ?? false,
    readAt: map['readAt'] != null 
        ? DateTime.fromMillisecondsSinceEpoch(map['readAt']) 
        : null,
  );
}
```

#### 1.2.2 Hive 모델 생성

**파일:** `lib/models/cache/cached_conversation.dart` (신규)

```dart
import 'package:hive/hive.dart';
import '../conversation.dart';

part 'cached_conversation.g.dart';

@HiveType(typeId: 2)
class CachedConversation extends HiveObject {
  @HiveField(0)
  final String id;
  
  @HiveField(1)
  final Map<String, dynamic> data;
  
  @HiveField(2)
  final DateTime cachedAt;
  
  CachedConversation({
    required this.id,
    required this.data,
    required this.cachedAt,
  });
  
  Conversation toConversation() => Conversation.fromMap(data, id);
  
  factory CachedConversation.fromConversation(Conversation conv) {
    return CachedConversation(
      id: conv.id,
      data: conv.toMap(),
      cachedAt: DateTime.now(),
    );
  }
}
```

**파일:** `lib/models/cache/cached_dm_message.dart` (신규)

```dart
import 'package:hive/hive.dart';
import '../dm_message.dart';

part 'cached_dm_message.g.dart';

@HiveType(typeId: 3)
class CachedDMMessage extends HiveObject {
  @HiveField(0)
  final String id;
  
  @HiveField(1)
  final Map<String, dynamic> data;
  
  @HiveField(2)
  final DateTime cachedAt;
  
  CachedDMMessage({
    required this.id,
    required this.data,
    required this.cachedAt,
  });
  
  DMMessage toMessage() => DMMessage.fromMap(data, id);
  
  factory CachedDMMessage.fromMessage(DMMessage msg) {
    return CachedDMMessage(
      id: msg.id,
      data: msg.toMap(),
      cachedAt: DateTime.now(),
    );
  }
}
```

#### 1.2.3 DM 캐시 매니저

**파일:** `lib/services/cache/dm_cache_manager.dart` (신규)

```dart
import 'package:hive/hive.dart';
import 'base_cache_manager.dart';
import 'cache_policy.dart';
import 'cache_feature_flags.dart';
import '../../models/conversation.dart';
import '../../models/dm_message.dart';
import '../../models/cache/cached_conversation.dart';
import '../../models/cache/cached_dm_message.dart';
import '../../utils/logger.dart';

class DMCacheManager extends BaseCacheManager<CachedConversation> {
  static final DMCacheManager _instance = DMCacheManager._internal();
  factory DMCacheManager() => _instance;
  DMCacheManager._internal();
  
  @override
  Box<CachedConversation>? get box {
    try {
      return Hive.isBoxOpen('conversations') 
          ? Hive.box<CachedConversation>('conversations') 
          : null;
    } catch (e) {
      return null;
    }
  }
  
  Box<CachedDMMessage>? get messageBox {
    try {
      return Hive.isBoxOpen('dm_messages') 
          ? Hive.box<CachedDMMessage>('dm_messages') 
          : null;
    } catch (e) {
      return null;
    }
  }
  
  @override
  CachePolicy get policy => CachePolicy.dm;
  
  @override
  bool _isDiskExpired(CachedConversation value) {
    return DateTime.now().difference(value.cachedAt) > policy.ttl;
  }
  
  /// 대화 목록 가져오기
  Future<List<Conversation>> getConversations() async {
    if (!CacheFeatureFlags.isDMCacheEnabled) return [];
    
    try {
      final cacheKey = 'conversation_list';
      final cached = await get(cacheKey);
      
      if (cached != null) {
        final conversations = (cached.data['conversations'] as List)
          .map((data) => Conversation.fromMap(data, data['id']))
          .toList();
        Logger.log('📦 대화 목록 캐시 히트: ${conversations.length}개');
        return conversations;
      }
    } catch (e) {
      Logger.error('대화 목록 캐시 읽기 실패: $e');
    }
    
    return [];
  }
  
  /// 대화 목록 저장
  Future<void> saveConversations(List<Conversation> conversations) async {
    if (!CacheFeatureFlags.isDMCacheEnabled) return;
    
    try {
      final cacheKey = 'conversation_list';
      await put(cacheKey, CachedConversation(
        id: cacheKey,
        data: {
          'conversations': conversations.map((c) => c.toMap()).toList(),
        },
        cachedAt: DateTime.now(),
      ));
      Logger.log('💾 대화 목록 캐시 저장: ${conversations.length}개');
    } catch (e) {
      Logger.error('대화 목록 캐시 저장 실패 (무시): $e');
    }
  }
  
  /// 특정 대화방의 메시지 가져오기
  Future<List<DMMessage>> getMessages(String conversationId) async {
    if (!CacheFeatureFlags.isDMCacheEnabled) return [];
    
    try {
      final msgBox = messageBox;
      if (msgBox == null || !msgBox.isOpen) return [];
      
      final cacheKey = 'messages_$conversationId';
      final cached = msgBox.get(cacheKey);
      
      if (cached != null && !_isMessageExpired(cached)) {
        final messages = (cached.data['messages'] as List)
          .map((data) => DMMessage.fromMap(data, data['id']))
          .toList();
        Logger.log('📦 메시지 캐시 히트: ${messages.length}개 (대화방: $conversationId)');
        return messages;
      }
    } catch (e) {
      Logger.error('메시지 캐시 읽기 실패: $e');
    }
    
    return [];
  }
  
  /// 특정 대화방의 메시지 저장
  Future<void> saveMessages(String conversationId, List<DMMessage> messages) async {
    if (!CacheFeatureFlags.isDMCacheEnabled) return;
    
    try {
      final msgBox = messageBox;
      if (msgBox == null || !msgBox.isOpen) return;
      
      final cacheKey = 'messages_$conversationId';
      await msgBox.put(cacheKey, CachedDMMessage(
        id: cacheKey,
        data: {
          'messages': messages.map((m) => m.toMap()).toList(),
        },
        cachedAt: DateTime.now(),
      ));
      Logger.log('💾 메시지 캐시 저장: ${messages.length}개 (대화방: $conversationId)');
    } catch (e) {
      Logger.error('메시지 캐시 저장 실패 (무시): $e');
    }
  }
  
  bool _isMessageExpired(CachedDMMessage value) {
    return DateTime.now().difference(value.cachedAt) > policy.ttl;
  }
  
  /// 특정 대화방 캐시 무효화
  void invalidateConversation(String conversationId) {
    if (!CacheFeatureFlags.isDMCacheEnabled) return;
    
    try {
      // 대화 목록 캐시 무효화
      invalidate(key: 'conversation_list');
      
      // 메시지 캐시 무효화
      final msgBox = messageBox;
      if (msgBox != null && msgBox.isOpen) {
        msgBox.delete('messages_$conversationId');
      }
      
      Logger.log('💾 DM 캐시 무효화 (대화방: $conversationId)');
    } catch (e) {
      Logger.error('DM 캐시 무효화 실패 (무시): $e');
    }
  }
}
```

#### 1.2.4 DMService 통합

**파일:** [lib/services/dm_service.dart](lib/services/dm_service.dart) 수정

```dart
// 1. import 추가
import 'dart:async';
import 'cache/dm_cache_manager.dart';
import 'cache/cache_feature_flags.dart';

// 2. 캐시 매니저 추가
class DMService {
  final DMCacheManager _cache = DMCacheManager();
  
  // 기존 메모리 캐시는 제거하고 DMCacheManager 사용
  // final Map<String, Conversation> _conversationCache = {}; // 제거
  // final Map<String, List<DMMessage>> _messageCache = {}; // 제거
  
  // 3. 캐시된 대화 목록 가져오기 메서드 추가
  Future<List<Conversation>> getCachedConversations() async {
    if (!CacheFeatureFlags.isDMCacheEnabled) return [];
    
    try {
      return await _cache.getConversations();
    } catch (e) {
      Logger.error('캐시된 대화 목록 가져오기 실패: $e');
      return [];
    }
  }
  
  // 4. Stream에서 캐시 업데이트
  Stream<List<Conversation>> getConversationsStream() {
    // 기존 Stream 로직에 캐시 업데이트 추가
    return _firestore
        .collection('conversations')
        .where(...)
        .snapshots()
        .asyncMap((snapshot) async {
          final conversations = ...;
          
          // 캐시 업데이트
          if (CacheFeatureFlags.isDMCacheEnabled) {
            unawaited(_cache.saveConversations(conversations));
          }
          
          return conversations;
        });
  }
  
  // 5. 메시지 전송 시 캐시 무효화
  Future<bool> sendMessage(...) async {
    final success = await _sendToFirestore(...);
    
    if (success && CacheFeatureFlags.isDMCacheEnabled) {
      _cache.invalidateConversation(conversationId);
    }
    
    return success;
  }
}
```

#### 1.2.5 CacheManager 업데이트

**파일:** [lib/services/cache/cache_manager.dart](lib/services/cache/cache_manager.dart) 수정

```dart
// 1. import 추가
import '../../models/cache/cached_conversation.dart';
import '../../models/cache/cached_dm_message.dart';

// 2. 어댑터 등록
Hive.registerAdapter(CachedConversationAdapter());
Hive.registerAdapter(CachedDMMessageAdapter());

// 3. 박스 열기
await Hive.openBox<CachedConversation>('conversations');
await Hive.openBox<CachedDMMessage>('dm_messages');

// 4. 삭제 메서드에 추가
await Hive.deleteBoxFromDisk('conversations');
await Hive.deleteBoxFromDisk('dm_messages');
```

---

## 2. 프로필 캐싱 구현

### 2.1 현재 상태 분석

**모델:** [lib/models/user_profile.dart](lib/models/user_profile.dart)
- `toFirestore()` 있음 (Timestamp 사용)
- `fromFirestore()` 있음
- 캐싱용 `toMap()`/`fromMap()` 필요

**문제점:**
- 프로필 조회 시 매번 Firestore 쿼리
- 친구 목록 조회 시 반복적인 프로필 쿼리
- 캐싱 없음

### 2.2 구현 계획

#### 2.2.1 모델에 toMap/fromMap 추가

**파일:** [lib/models/user_profile.dart](lib/models/user_profile.dart)

```dart
// 캐싱용 Map 변환 메서드 추가
Map<String, dynamic> toMap() {
  return {
    'uid': uid,
    'displayName': displayName,
    'photoURL': photoURL,
    'nickname': nickname,
    'nationality': nationality,
    'email': email,
    'university': university,
    'friendsCount': friendsCount,
    'incomingCount': incomingCount,
    'outgoingCount': outgoingCount,
    'createdAt': createdAt.millisecondsSinceEpoch,
    'updatedAt': updatedAt.millisecondsSinceEpoch,
  };
}

factory UserProfile.fromMap(Map<String, dynamic> map) {
  return UserProfile(
    uid: map['uid'] ?? '',
    displayName: map['displayName'] ?? '',
    photoURL: map['photoURL'],
    nickname: map['nickname'],
    nationality: map['nationality'],
    email: map['email'],
    university: map['university'],
    friendsCount: map['friendsCount'] ?? 0,
    incomingCount: map['incomingCount'] ?? 0,
    outgoingCount: map['outgoingCount'] ?? 0,
    createdAt: DateTime.fromMillisecondsSinceEpoch(map['createdAt']),
    updatedAt: DateTime.fromMillisecondsSinceEpoch(map['updatedAt']),
  );
}
```

#### 2.2.2 Hive 모델 생성

**파일:** `lib/models/cache/cached_user_profile.dart` (신규)

```dart
import 'package:hive/hive.dart';
import '../user_profile.dart';

part 'cached_user_profile.g.dart';

@HiveType(typeId: 4)
class CachedUserProfile extends HiveObject {
  @HiveField(0)
  final String uid;
  
  @HiveField(1)
  final Map<String, dynamic> data;
  
  @HiveField(2)
  final DateTime cachedAt;
  
  CachedUserProfile({
    required this.uid,
    required this.data,
    required this.cachedAt,
  });
  
  UserProfile toProfile() => UserProfile.fromMap(data);
  
  factory CachedUserProfile.fromProfile(UserProfile profile) {
    return CachedUserProfile(
      uid: profile.uid,
      data: profile.toMap(),
      cachedAt: DateTime.now(),
    );
  }
}
```

#### 2.2.3 프로필 캐시 매니저

**파일:** `lib/services/cache/profile_cache_manager.dart` (신규)

```dart
import 'package:hive/hive.dart';
import 'base_cache_manager.dart';
import 'cache_policy.dart';
import 'cache_feature_flags.dart';
import '../../models/user_profile.dart';
import '../../models/cache/cached_user_profile.dart';
import '../../utils/logger.dart';

class ProfileCacheManager extends BaseCacheManager<CachedUserProfile> {
  static final ProfileCacheManager _instance = ProfileCacheManager._internal();
  factory ProfileCacheManager() => _instance;
  ProfileCacheManager._internal();
  
  @override
  Box<CachedUserProfile>? get box {
    try {
      return Hive.isBoxOpen('profiles') 
          ? Hive.box<CachedUserProfile>('profiles') 
          : null;
    } catch (e) {
      return null;
    }
  }
  
  @override
  CachePolicy get policy => CachePolicy.profile;
  
  @override
  bool _isDiskExpired(CachedUserProfile value) {
    return DateTime.now().difference(value.cachedAt) > policy.ttl;
  }
  
  /// 프로필 가져오기
  Future<UserProfile?> getProfile(String uid) async {
    if (!CacheFeatureFlags.isProfileCacheEnabled) return null;
    
    try {
      final cached = await get(uid);
      if (cached != null) {
        Logger.log('📦 프로필 캐시 히트: $uid');
        return cached.toProfile();
      }
    } catch (e) {
      Logger.error('프로필 캐시 읽기 실패: $e');
    }
    
    return null;
  }
  
  /// 프로필 저장
  Future<void> saveProfile(UserProfile profile) async {
    if (!CacheFeatureFlags.isProfileCacheEnabled) return;
    
    try {
      await put(profile.uid, CachedUserProfile.fromProfile(profile));
      Logger.log('💾 프로필 캐시 저장: ${profile.uid}');
    } catch (e) {
      Logger.error('프로필 캐시 저장 실패 (무시): $e');
    }
  }
  
  /// 여러 프로필 한번에 저장
  Future<void> saveProfiles(List<UserProfile> profiles) async {
    if (!CacheFeatureFlags.isProfileCacheEnabled) return;
    
    try {
      for (final profile in profiles) {
        await put(profile.uid, CachedUserProfile.fromProfile(profile));
      }
      Logger.log('💾 프로필 캐시 일괄 저장: ${profiles.length}개');
    } catch (e) {
      Logger.error('프로필 캐시 일괄 저장 실패 (무시): $e');
    }
  }
}
```

#### 2.2.4 사용 예시

프로필을 조회하는 모든 곳에서 캐시를 먼저 확인:

```dart
// 예: 친구 목록 조회 시
Future<UserProfile?> getUserProfile(String uid) async {
  // 1. 캐시 확인
  final cached = await _profileCache.getProfile(uid);
  if (cached != null) return cached;
  
  // 2. Firestore 조회
  final doc = await _firestore.collection('users').doc(uid).get();
  if (!doc.exists) return null;
  
  final profile = UserProfile.fromFirestore(doc);
  
  // 3. 캐시 저장
  await _profileCache.saveProfile(profile);
  
  return profile;
}
```

---

## 3. 성능 최적화

### 3.1 캐시 크기 모니터링

**파일:** `lib/services/cache/cache_monitor.dart` (신규)

```dart
import 'package:hive/hive.dart';
import 'cache_manager.dart';
import '../../utils/logger.dart';

class CacheMonitor {
  static final CacheMonitor _instance = CacheMonitor._internal();
  factory CacheMonitor() => _instance;
  CacheMonitor._internal();
  
  /// 전체 캐시 크기 확인 (MB)
  Future<double> getTotalCacheSize() async {
    try {
      double totalSize = 0;
      
      final boxes = ['metadata', 'posts', 'comments', 'conversations', 'dm_messages', 'profiles'];
      
      for (final boxName in boxes) {
        if (CacheManager.isBoxOpen(boxName)) {
          final box = Hive.box(boxName);
          // 대략적인 크기 계산 (각 항목당 평균 크기 추정)
          totalSize += box.length * 0.01; // 10KB per item (추정)
        }
      }
      
      return totalSize;
    } catch (e) {
      Logger.error('캐시 크기 계산 실패: $e');
      return 0;
    }
  }
  
  /// 캐시 통계
  Map<String, dynamic> getStats() {
    final stats = CacheManager.getStats();
    stats['totalSizeMB'] = getTotalCacheSize();
    return stats;
  }
  
  /// 오래된 캐시 정리
  Future<void> cleanOldCache() async {
    try {
      Logger.log('🧹 오래된 캐시 정리 시작...');
      
      // 각 박스에서 오래된 항목 삭제
      // (BaseCacheManager의 TTL 체크 활용)
      
      Logger.log('✅ 오래된 캐시 정리 완료');
    } catch (e) {
      Logger.error('캐시 정리 실패: $e');
    }
  }
}
```

### 3.2 백그라운드 정리 작업

**파일:** [lib/main.dart](lib/main.dart) 수정

```dart
// 앱 시작 시 백그라운드 정리 작업 예약
void main() async {
  // ... 기존 초기화 코드
  
  // 캐시 시스템 초기화
  await CacheManager.initialize();
  
  // 백그라운드 정리 작업 (5분 후 실행)
  Future.delayed(Duration(minutes: 5), () {
    CacheMonitor().cleanOldCache();
  });
  
  runApp(MyApp());
}
```

### 3.3 메모리 최적화

**파일:** [lib/services/cache/base_cache_manager.dart](lib/services/cache/base_cache_manager.dart) 개선

```dart
// 메모리 사용량 모니터링 추가
int get memoryUsageEstimate {
  // 각 항목당 대략적인 크기 계산
  return _memoryCache.length * 1024; // 1KB per item (추정)
}

// 메모리 압박 시 강제 정리
void forceMemoryCleanup() {
  if (_memoryCache.length > policy.maxMemoryItems ~/ 2) {
    final toRemove = _memoryCache.length - (policy.maxMemoryItems ~/ 2);
    final sortedKeys = _cacheTime.entries
      .toList()
      ..sort((a, b) => a.value.compareTo(b.value));
    
    for (int i = 0; i < toRemove; i++) {
      final key = sortedKeys[i].key;
      _memoryCache.remove(key);
      _cacheTime.remove(key);
    }
    
    Logger.log('🧹 메모리 캐시 강제 정리: $toRemove개 항목 제거');
  }
}
```

### 3.4 디스크 캐시 압축

**파일:** [lib/services/cache/cache_manager.dart](lib/services/cache/cache_manager.dart) 개선

```dart
/// 디스크 캐시 압축 (오래된 항목 삭제)
static Future<void> compactCache() async {
  try {
    Logger.log('🗜️ 디스크 캐시 압축 시작...');
    
    final boxes = ['posts', 'comments', 'conversations', 'dm_messages', 'profiles'];
    
    for (final boxName in boxes) {
      if (isBoxOpen(boxName)) {
        final box = Hive.box(boxName);
        await box.compact();
        Logger.log('✅ $boxName 박스 압축 완료');
      }
    }
    
    Logger.log('✅ 디스크 캐시 압축 완료');
  } catch (e) {
    Logger.error('디스크 캐시 압축 실패: $e');
  }
}
```

### 3.5 성능 메트릭 수집

**파일:** `lib/services/cache/cache_metrics.dart` (신규)

```dart
class CacheMetrics {
  static final CacheMetrics _instance = CacheMetrics._internal();
  factory CacheMetrics() => _instance;
  CacheMetrics._internal();
  
  int _cacheHits = 0;
  int _cacheMisses = 0;
  int _networkRequests = 0;
  
  void recordHit() => _cacheHits++;
  void recordMiss() => _cacheMisses++;
  void recordNetworkRequest() => _networkRequests++;
  
  double get hitRate {
    final total = _cacheHits + _cacheMisses;
    return total > 0 ? _cacheHits / total : 0;
  }
  
  Map<String, dynamic> getReport() {
    return {
      'cacheHits': _cacheHits,
      'cacheMisses': _cacheMisses,
      'networkRequests': _networkRequests,
      'hitRate': '${(hitRate * 100).toStringAsFixed(1)}%',
      'networkReduction': '${((1 - networkRequests / (cacheHits + cacheMisses + networkRequests)) * 100).toStringAsFixed(1)}%',
    };
  }
  
  void reset() {
    _cacheHits = 0;
    _cacheMisses = 0;
    _networkRequests = 0;
  }
}
```

---

## 4. 구현 순서

### Phase 3-1: DM 캐싱 (1주)
1. ✅ Conversation/DMMessage 모델에 toMap/fromMap 추가
2. ✅ CachedConversation/CachedDMMessage Hive 모델 생성
3. ✅ 코드 생성 (`flutter packages pub run build_runner build`)
4. ✅ DMCacheManager 구현
5. ✅ CacheManager에 어댑터/박스 등록
6. ✅ DMService에 캐시 통합
7. ✅ 테스트

### Phase 3-2: 프로필 캐싱 (3일)
1. ✅ UserProfile 모델에 toMap/fromMap 추가
2. ✅ CachedUserProfile Hive 모델 생성
3. ✅ 코드 생성
4. ✅ ProfileCacheManager 구현
5. ✅ CacheManager에 어댑터/박스 등록
6. ✅ 프로필 조회하는 모든 곳에 캐시 적용
7. ✅ 테스트

### Phase 3-3: 성능 최적화 (3일)
1. ✅ CacheMonitor 구현
2. ✅ 백그라운드 정리 작업 추가
3. ✅ 메모리 최적화
4. ✅ 디스크 캐시 압축
5. ✅ CacheMetrics 구현
6. ✅ 성능 테스트 및 튜닝

---

## 5. 예상 효과

### 성능 개선
- **DM 로딩**: 2초 → 0.2초 (10배)
- **프로필 조회**: 0.5초 → 0.05초 (10배)
- **네트워크 요청**: 전체 90% 감소
- **메모리 사용**: 최적화로 30% 감소

### 사용자 경험
- 모든 화면 즉시 로딩
- 오프라인에서도 최근 대화 확인 가능
- 데이터 사용량 대폭 감소
- 배터리 수명 연장

### 저장 공간
- DM 500개: ~300KB
- 프로필 100명: ~200KB
- 총 캐시 크기: 3-7MB (매우 적음)

---

## 6. 주의사항

### 데이터 일관성
- DM은 실시간성이 중요하므로 TTL을 1시간으로 설정
- 새 메시지 수신 시 즉시 캐시 무효화
- 읽음 상태 변경 시 캐시 업데이트

### 메모리 관리
- 메모리 캐시는 최소한으로 유지
- 디스크 캐시를 주로 활용
- 앱 백그라운드 진입 시 메모리 캐시 정리

### 보안
- 민감한 DM 내용은 암호화 고려
- 로그아웃 시 모든 캐시 삭제
- 프로필 정보는 공개 정보만 캐시

---

## 7. 테스트 계획

### 단위 테스트
- 각 CacheManager의 CRUD 동작
- TTL 만료 확인
- 무효화 로직

### 통합 테스트
- 캐시 히트/미스 시나리오
- 네트워크 오류 시 폴백
- 동시성 테스트

### 성능 테스트
- 1000개 메시지 로딩 시간
- 100명 프로필 조회 시간
- 메모리 사용량 측정
- 디스크 I/O 측정

---

## 8. 롤아웃 전략

### 1단계: 개발 환경 (1주)
- Feature Flag: `dm_cache_enabled = false`
- Feature Flag: `profile_cache_enabled = false`
- 내부 테스트

### 2단계: 베타 테스터 (1주)
- Feature Flag: `dm_cache_enabled = true` (10% 사용자)
- Feature Flag: `profile_cache_enabled = true` (10% 사용자)
- 피드백 수집

### 3단계: 전체 배포 (1주)
- Feature Flag: 100% 활성화
- 모니터링
- 성능 메트릭 수집

---

## 9. 완료 기준

✅ **DM 캐싱**
- 대화 목록 캐시 동작
- 메시지 캐시 동작
- 새 메시지 시 캐시 무효화
- 오프라인에서 최근 대화 확인 가능

✅ **프로필 캐싱**
- 프로필 조회 시 캐시 우선
- 친구 목록 로딩 속도 10배 향상
- 메모리 사용량 최적화

✅ **성능 최적화**
- 전체 캐시 크기 < 10MB
- 메모리 사용량 < 50MB
- 네트워크 요청 90% 감소
- 앱 시작 시간 < 1초

---

이 계획으로 **완전한 오프라인 우선 앱**을 만들 수 있습니다! 🚀










