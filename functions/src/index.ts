 ...
 */
export const createGeneralEmailSignup = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (createdNewAuthUser && authUser) {
      ...
      await db.collection(COL.users).doc(authUser.uid).delete().catch(() => {});
    }
