...
class DMUserInfo {
  ...
  /// 한양 인증(Univ.) 여부. users/{uid} 문서 기준으로 계산한다.
  /// - `hanyangVerified == true`이면 true
  /// - `hanyangEmail`이 @hanyang.ac.kr 도메인이면 true
  /// - 그 외 false
  final bool isHanyangVerified;
...
class UserInfoCacheService {
  ...
  Future<DMUserInfo?> getUserInfo(
    ...
    try {
      ...
      final userInfo = DMUserInfo(
        ...
        isHanyangVerified: _resolveHanyangVerified(data),
      );
    ...
    return _watchStreams.putIfAbsent(userId, () {
      return _firestore
        ...
        return prev.nickname == next.nickname &&
            ...
            prev.isHanyangVerified == next.isHanyangVerified;
      }).handleError((e) {
  ...
  /// 문서 데이터 기준으로 한양 인증 여부를 계산한다.
  ///
  /// 규칙:
  /// - `hanyangVerified == true` 이면 true
  /// - `hanyangEmail` 값이 @hanyang.ac.kr 이면 true
  /// - 그 외 false
