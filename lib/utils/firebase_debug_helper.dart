...
class FirebaseDebugHelper {
  ...
  Future<Map<String, dynamic>> testFirebaseStorage() async {
    try {
      ...
      try {
        final testFilePath =
            'debug/test_${DateTime.now().millisecondsSinceEpoch}.txt';
        final testContent = 'Firebase Storage Debug Test: ${DateTime.now()}';

        final uploadBytes = await _storage
            .ref(testFilePath)
            .putString(testContent, format: PutStringFormat.raw);
      ...
      return {'error': e.toString(), 'storage_initialized': _storage != null};
    }
  }
  ...
  Future<bool> testSecurityRules() async {
    try {
      ...
      final testPath =
          'security_test_${DateTime.now().millisecondsSinceEpoch}.txt';
      await _storage
          .ref(testPath)
          .putString('Security test', format: PutStringFormat.raw);
