...
class _MockFirebaseFirestoreHostApi extends Mock
  ...
  Future<void> transactionStoreResult(
    ...
    if (!storeResultCalled.isCompleted) storeResultCalled.complete();
...
void main() {
  ...
  const codec = StandardMethodCodec(FirestoreMessageCodec());
  ...
  setUpAll(() async {
    app = await Firebase.initializeApp(
      name: 'transaction-race-test-app',
  ...
  setUp(() {
    ...
    messenger.setMockMessageHandler(channelName, (ByteData? message) async {
      if (!eventChannelListened.isCompleted) eventChannelListened.complete();
  ...
  test(
    'completes once when native error races transaction failure storage',
    () async {
      ...
      unawaited(
        messenger.handlePlatformMessage(
          ...
          codec.encodeSuccessEnvelope(<String, Object?>{
            'appName': 'transaction-race-test-app',
          }),
          (_) {},
        ),
      );
